...
- 生成模型：仅解码器模型
  - 生成式LLM作为一种序列到序列(sequence-to-sequence, Seq2Seq)的文本生成系统，其核心机制是接收文本输入并尝试自动补全。尽管自动补全功能很实用，但这类模型真正的强大之处在于经过训练称为聊天机器人。与其只是补全文本，不如将它们训练得能够回答问题。通过微调这些模型，我们可以创建能够遵循人类指示的**指令模型**(instruct model)或**对话模型**(chat model)。
- 表示模型：仅编码器模型
- LLM的训练范式
  - 创建LLM通常包含至少两步骤：
