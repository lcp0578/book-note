...
- 软件过程模型
	...
	- **基于构件的开发模型（CBSD）**：特点是增强了可复用性，在系统开发过程中，会构建一个构件库，供其他系统复用，因此可以提高可靠性，节省时间和成本。
...
- 需求工程
	...
	- 需求定义
		- 需求定义的方法如下：
			- **严格定义**也称为预先定义，需求的严格定义建立在以下的基础假设之上：
				- 所有需求都能够被预先定义。
				- 开发人员与用户之间能够准确而清晰地交流。
				- 采用图形（或文字）可以充分体现最终系统。
...
- 系统设计
	- 处理流程设计
		...
		- 业务流程重组（BPR）。BPR是对企业的业务流程进行根本性的再思考和彻底性的再设计，从而获得可以用诸如成本、质量、服务和速度等方面的业绩来衡量的显著性成就。
			- BPR设计原则、系统规划和步骤如图：
...
- 净室软件工程（CSE）
	...
	- 净室软件工程在使用过程中的一些缺点。
		...
		- CSE也会带有传统软件工程的一些弊端。
- 基于构件的软件工程（CBSE）
	- 基于构件的软件工程（CBSE）是一种基于分布对象技术、强调通过可复用构件设计有构造软件系统的软性复用途径。
	...
	- 构件模型定义了构件实现、文档化以及开发的标准，其包含的模型要素为：
		...
		- 使用信息
		- 部署
	- CBSE过程是支持基于构件组装的软件开发过程，过程中的六个主要活动：
		- 系统需求概览
		- 识别候选构件
		- 根据发现的构件修改需求
		- 体系结构设计
		- 构件定制与适配
		- 组装构件创建系统
	- 构件组装是指构件相互直接集成或是专门编写的“胶水代码”将它们整合在一起来创造一个系统或另一个构件的过程。常见的组装构件有以下三种组装方式。
		- 顺序组装
		- 层次组装
		- 叠加组装
