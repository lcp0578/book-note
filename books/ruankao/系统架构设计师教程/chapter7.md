...
- 7.3.6 独立构件体系结构风格
	...
	- 2.事件系统体系结构风格
		...
		- 基于事件的隐式调用风格的主要特点是事件触发者并不知道哪些构件会被这些事件影响。这使得不能假定构件的处理顺序，甚至不知道哪些过程会被调用，因此，许多隐式调用的系统也包含显示调用作为构件交互的补充形式。
#### 7.4 软件架构复用
- 7.4.1 软件架构复用的定义及分类
...
- 7.5.3 参与DSSA的人员
	...
	- 4.领域实现人员
- 7.5.4 DSSA的建立过程
	- DSSA的简历过程分为5个阶段，每个阶段可以进一步划分为一些步骤或子阶段。每个阶段包括一组需要回答的问题，一组需要的输入，一组将产生的输出和验证标准。
	- 本过程是并发的、递归的、反复的。或者可以说，它是螺旋模型。
	- 过程本过程可能需要对每个阶段经历几遍，每次增加更多的细节。
		- 定义领域范围
		- 定义领域特定的元素
		- 定义领域特定的设计和实现需求约束
		- 定义领域模型和体系结构
